...
// ---------------------------------------------------------
// 1. XỬ LÝ MÃ GIẢM GIÁ (Giữ nguyên)
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// 2. CẬP NHẬT GIỎ HÀNG (Giữ nguyên)
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// 3. XỬ LÝ THANH TOÁN (Có sửa đổi để nhận địa chỉ Cửa hàng)
// ---------------------------------------------------------
if (isset($_POST['thanhtoan'])) {
    ...
    // [MỚI] Xử lý địa chỉ dựa trên hình thức giao hàng
    $shipping_method = $_POST['shipping_method'] ?? 'home';
    
    if ($shipping_method == 'store') {
        // Nếu lấy tại cửa hàng -> Địa chỉ là tên chi nhánh
        $branch = $_POST['store_branch'] ?? '';
        $fullAddress = "Nhận tại: " . $branch;
        // Các biến tỉnh/xã để trống hoặc gán mặc định để không lỗi
        $tinh = ""; $xa = ""; $sonha = ""; 
    } else {
        // Nếu giao tận nơi -> Lấy địa chỉ như cũ
        $tinh = $_POST['tinh'] ?? '';
        $xa = $_POST['xa'] ?? '';
        $sonha = $_POST['sonha'] ?? '';
    ...
    <form class="ttvc" method="post" name="infor" id="infor">
        ...
        <div id="store-address" style="display: none; width: 96%; margin-bottom: 15px;">
            ...
            <select name="store_branch" id="store_branch" style="width: 100%; padding: 10px;">
                <option value="CS1: 20 Võ Oanh, P.25, Q.Bình Thạnh, TP.HCM">CS1: 20 Võ Oanh, P.25, Q.Bình Thạnh, TP.HCM</option>
                <option value="CS2: 123 Nguyễn Văn Cừ, Q.5, TP.HCM">CS2: 123 Nguyễn Văn Cừ, Q.5, TP.HCM</option>
                <option value="CS3: 456 Lê Duẩn, Q.Hải Châu, Đà Nẵng">CS3: 456 Lê Duẩn, Q.Hải Châu, Đà Nẵng</option>
                <option value="CS4: 789 Cầu Giấy, Hà Nội">CS4: 789 Cầu Giấy, Hà Nội</option>
            </select>
        </div>
...
var currentShip = 0;
var shippingMethod = 'home'; // Mặc định là giao tận nơi
...
// [MỚI] Hàm chuyển đổi giữa Giao tận nơi và Lấy tại cửa hàng
window.toggleShipping = function(method) {
    shippingMethod = method;
    if (method === 'store') {
        // Ẩn form địa chỉ, hiện form chọn cửa hàng
        $('#delivery-address').hide();
        $('#store-address').show();
        
        // Bỏ bắt buộc nhập địa chỉ (để không lỗi khi submit)
        $('#tinh, #huyen, #xa, #sonha').prop('required', false);
        
        // Lấy tại cửa hàng thì ship = 0
        currentShip = 0;
        updateShippingUI();
    } else {
        // Hiện form địa chỉ, ẩn form chọn cửa hàng
        $('#delivery-address').show();
        $('#store-address').hide();
        
        // Bật lại bắt buộc nhập
        $('#tinh, #huyen, #xa, #sonha').prop('required', true);
        
        // Tính lại ship dựa trên tỉnh đã chọn
        calculateShippingFee();
    }
}

// Hàm cập nhật giao diện ship
...
// Hàm tính phí ship (Tách riêng để tái sử dụng)
function calculateShippingFee() {
    // [MỚI] Nếu tổng tiền >= 3 triệu -> Free ship
...
$(document).ready(function() {
    ...
    $.getJSON('https://esgoo.net/api-tinhthanh/1/0.htm', function(data_tinh) {
        ...
        $("#tinh").change(function() {
            // Chỉ tính ship nếu đang chọn Giao tận nơi
    ...
    // [MỚI] Kiểm tra Free Ship ngay khi tải trang (nếu đơn > 3tr)
    calculateShippingFee();
});
</script>
